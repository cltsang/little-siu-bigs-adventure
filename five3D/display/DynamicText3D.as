	...
	public class DynamicText3D extends Sprite3D {
		...
		private function createGlyphs(text:String):void {
			...
			for (var i:int = 0; i < text.length; i++){
				if ((text.charAt(i) != __text.charAt(i)) || !identical){
					createGlyph(i, text.charAt(i));
					identical = false;
				}
			}
		}
		...
		private function placeGlyphs():void {
			...
			for (var i:int = 0; i < numChildren; i++) {
				...
				if (i == numChildren-1) __textwidth = offset+__typography.__widths[__text.charAt(i)]*__size/100;
